...
import { ArrowLeft, ArrowRight, Camera, CheckCircle2, FilePlus2, HeartPulse, ImagePlus, Pencil, Recycle, RefreshCcw, Search, Sparkles, Trash2, Wrench, BadgeIndianRupee } from 'lucide-react';
...
const steps = ['Identify device', 'Device information', 'Request details', 'Issue evidence', 'Review & submit'];
...
export function RequestEnhanced() {
  ...
  useEffect(() => {
    ...
    if (match) { setDevice(match); setSerial(match.serialNumber); setForm(current => ({ ...current, deviceId: match._id, location: locations.includes(match.location) ? match.location : current.location })); setStep(1); }
  ...
  const canContinue = [Boolean(form.deviceId), Boolean(form.deviceId), Boolean(form.description.trim()), true, Boolean(form.deviceId && form.description.trim())];
  ...
  const category = categories.find(item => item.value === form.category) || categories[0];

  return <Shell title="Raise Request">
    <PageHeader title="Raise Request" description="Tell us which device needs attention and what's happening. It takes about two minutes." />
    {aiRequest && <InlineAlert tone="info" title="Prepared from your AI Support conversation">Review the details below before submitting.</InlineAlert>}
    <Stepper steps={steps} current={step} onStepClick={setStep} />
    {error && <InlineAlert title={error} />}

    <div className="wizard">
      <div className="wizard-main">
        <Card className="wizard-card" title={steps[step]} description={[
          'Enter or scan the serial number. Device details are filled in automatically.',
          'Confirm this is the right device before continuing.',
          'Choose the request type and describe what is needed.',
          'Add photos of the issue and mark the damaged area. This step is optional.',
          ...
          {step === 0 && (loadError ? <ErrorState compact title="Unable to load your devices" message={friendlyError(loadError)} onRetry={loadDevices} />
            : !devices ? <div className="stack-12"><Skeleton height={44} /><Skeleton height={80} /></div>
            : <div className="stack-16">
              <div className="lookup-row">
                <Field label="Serial number" hint="You can also search by model, asset ID or employee." error={touched && !form.deviceId ? 'Select a device to continue.' : undefined}>
                  {props => <div className="lookup-input"><Search size={17} aria-hidden="true" /><input {...props} className="mono-input" value={serial} onChange={event => lookup(event.target.value)} placeholder="e.g. APL-DEMO-0001" autoComplete="off" data-autofocus /></div>}
                </Field>
                <Button icon={Camera} className="btn-lg" onClick={() => setScanOpen(true)}>Scan barcode / QR</Button>
              </div>
              {device ? <div className="device-card">
                <span className="device-card-icon"><DeviceIcon type={device.deviceType} model={device.model} /></span>
                <div className="device-card-body"><strong>{device.model}</strong><span>{[device.serialNumber, device.assetId, device.employeeName || 'Unassigned', device.location].filter(Boolean).join(' · ')}</span></div>
                ...
                <div className="device-options" role="listbox" aria-label="Matching devices">{suggestions.map(item => <button type="button" role="option" aria-selected="false" key={item._id} className="device-option" onClick={() => { setSerial(item.serialNumber); selectDevice(item); }}>
                  <span><strong>{item.model}</strong><span className="mono">{item.serialNumber}</span> · <span>{item.employeeName || 'Unassigned'}</span></span><ArrowRight size={16} aria-hidden="true" />
                </button>)}</div>
          ...
          {step === 1 && device && <div className="stack-16">
            <div className="device-card"><span className="device-card-icon"><DeviceIcon type={device.deviceType} model={device.model} /></span><div className="device-card-body"><strong>{device.model}</strong><span className="mono">{device.serialNumber}</span></div><Badge dot>{device.deviceAllocationStatus || 'Assigned'}</Badge></div>
            <InfoList columns={2} items={[['Device type', device.deviceType], ['Asset ID', device.assetId], ['Employee', device.employeeName || 'Unassigned'], ['Department', device.department], ['Location', device.location], ['Purchase date', formatDate(device.purchaseDate, '')], ['Warranty', <Badge>{device.warrantyStatus}</Badge>], ['AMC', <Badge>{device.amcStatus}</Badge>]]} />
          </div>}

          {step === 2 && <div className="stack-16">
            <div className="field"><span className="field-label">Request category</span>
              <div className="choice-grid" role="group" aria-label="Request category">{categories.map(item => <button type="button" key={item.value} className="choice" aria-pressed={form.category === item.value} onClick={() => setForm({ ...form, category: item.value })}><item.icon size={18} aria-hidden="true" /><strong>{item.value}</strong><span>{item.hint}</span></button>)}</div>
            </div>
            <div className="form-grid">
              ...
              <Field label="Priority" hint="Sets the resolution target for this request.">{props => <select {...props} value={form.priority} onChange={event => setForm({ ...form, priority: event.target.value })}>{priorities.map(item => <option key={item}>{item}</option>)}</select>}</Field>
              <Field className="span-2" label="Description" required hint="What is happening, when it started, and anything you've already tried." error={touched && !form.description.trim() ? 'Describe the issue to continue.' : undefined}>
          ...
          {step === 3 && <div className="stack-16">
            <label className="dropzone">
              <span className="dropzone-icon" aria-hidden="true"><ImagePlus size={18} /></span>
              <strong>Upload issue photos</strong>
              <span>PNG, JPG or WebP, up to 5 MB each. Click or drop files here.</span>
          ...
          {step === 4 && <div className="review-summary">
            ...
            <div className="review-section"><div className="review-section-head"><h3>Request</h3><Button size="sm" variant="ghost" onClick={() => setStep(2)}>Edit</Button></div>
              <InfoList columns={2} items={[['Category', form.category], ['Issue type', form.issueType], ['Priority', <Badge>{form.priority}</Badge>], ['Location', form.location], ['Preferred date', formatDate(form.preferredServiceDate, 'No preference')]]} />
              <p className="description-text" style={{ marginTop: 8 }}>{form.description}</p></div>
            <div className="review-section"><div className="review-section-head"><h3>Evidence</h3><Button size="sm" variant="ghost" onClick={() => setStep(3)}>Edit</Button></div>
              <p className="text-muted">{files.length ? `${files.length} photo${files.length === 1 ? '' : 's'}${annotated.length ? `, ${annotated.length} annotated` : ''}` : 'No photos attached'}</p></div>
          </div>}
        </Card>
        ...
        <div className="card"><div className="wizard-footer" style={{ borderTop: 0, borderRadius: 'var(--radius-lg)' }}>
          ...
          {step < steps.length - 1 ? <Button variant="primary" onClick={next}>{step === 3 && !files.length ? 'Skip for now' : 'Continue'}<ArrowRight size={16} aria-hidden="true" /></Button>
            : <Button variant="primary" icon={CheckCircle2} onClick={submit} disabled={submitting || !canContinue[4]}>{submitting ? 'Submitting…' : 'Submit request'}</Button>}
        </div></div>
      </div>

      <aside className="wizard-aside">
        {device && <Card title="Selected device"><div className="stack-12">
          <div className="person-cell"><span className="asset-icon" style={{ width: 40, height: 40 }}><DeviceIcon type={device.deviceType} model={device.model} /></span><div><strong>{device.model}</strong><span className="cell-sub mono">{device.serialNumber}</span></div></div>
          <div className="row"><Badge>{`Warranty ${device.warrantyStatus || '—'}`}</Badge><Badge>{`AMC ${device.amcStatus || '—'}`}</Badge></div>
        </div></Card>}
        <Card title="How it works"><ol className="guide-list">
          <li className="guide-item"><span className="num">1</span><div><strong>Your request reaches iPlanet Service</strong>With the full device and coverage record attached.</div></li>
          <li className="guide-item"><span className="num">2</span><div><strong>An engineer is assigned</strong>Response and resolution targets are set by priority.</div></li>
          <li className="guide-item"><span className="num">3</span><div><strong>Track every update</strong>Progress appears in Service Requests and Notifications.</div></li>
        </ol></Card>
        <Card title={<span className="row"><category.icon size={16} aria-hidden="true" />{category.value}</span>}><p className="text-muted text-small">Not sure what's wrong? <button type="button" className="btn-link" onClick={ai.open}><Sparkles size={13} aria-hidden="true" style={{ verticalAlign: '-2px' }} /> Ask AI Support</button> to troubleshoot first.</p></Card>
      </aside>
    </div>
